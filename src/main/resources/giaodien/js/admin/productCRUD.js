document.addEventListener("DOMContentLoaded", () => {
	...
	function openAddModel() {
		...
		resetPreviewImage("previewImageAdd")
		// Hiển thị modal
		document.getElementById("addProductModal").style.display = "block";
	}
	const btn_add = document.querySelector('.btn.btn-add');
	btn_add.addEventListener("click", () => {
		openAddModel();
		previewImage("fileInputAdd", "previewImageAdd");
	})

	const closeAddModalBtn = document.getElementById("closeAddModel");
	...
	function updateRowInTable(updatedProduct) {
		...
		row.children[7].textContent = updatedProduct.description;
	...
	// add sự kiện preview ảnh khi chọn
	...
	// mở modal mới, ảnh cũ không còn bị lưu từ lần trước
	function resetPreviewImage(previewId) {
	...
	formEdit.addEventListener("submit", (event) => {
		...
		updateProduct();
		document.getElementById("editProductModal").style.display = "none"; // đóng modal
	...
	function openEditModal(product) {
		...
		if (product.imagePath) {
			previewImg.src = product.imagePath;
	...
	productTableBody.addEventListener("click", (event) => {
		...
		if (target.classList.contains("edit-btn")) {
			...
			previewImage("fileInputEdit", "previewImageEdit");
